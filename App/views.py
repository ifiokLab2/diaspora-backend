...
class ProductDetailView(APIView):
    ...
    def get(self, request, slug):
        ...
        product = get_object_or_404(
            Product.objects.select_related('category', 'country', 'city').prefetch_related('images'), 
            slug=slug
        )
        ...
        # Related products also need their Cloudinary URLs
        related_products = Product.objects.filter(
            category=product.category
        ).exclude(id=product.id).select_related('country', 'city')
...
class ProductListCreateAPIView(APIView):
    permission_classes = [IsAuthenticated]
    # MultiPartParser is essential for handling image/file uploads
    parser_classes = [parsers.MultiPartParser, parsers.FormParser]

    def get(self, request):
        """List products belonging to the logged-in seller with optimized queries"""
        products = Product.objects.filter(seller=request.user).select_related(
            'country', 'city', 'category'
        ).prefetch_related('images').order_by('-created_date') # Added prefetch_related for gallery

        paginator = StandardResultsSetPagination()
        result_page = paginator.paginate_queryset(products, request)
        
        # Use your detailed ProductSerializer for the list view
        serializer = ProductSerializer(result_page, many=True, context={'request': request})
        print('serializer.data:',serializer.data)
        return paginator.get_paginated_response(serializer.data)

    def post(self, request):
        """Create a product and handle multiple gallery images"""
        # We pass the full request to the serializer context
        
        serializer = ProductCreateSerializer(data=request.data, context={'request': request})
        
        if serializer.is_valid():
            # The 'create' method in our updated Serializer now handles 
            # both the product and the gallery images automatically.
    ...
    def patch(self, request):
        ...
        # Use ProductCreateSerializer for updates if you want to allow uploading 
        # new gallery images during a patch.
    ...
    def delete(self, request):
        """Delete product and its associated Cloudinary images via signals"""
        product_id = request.query_params.get('id')
        try:
            product = Product.objects.get(id=product_id, seller=request.user)
            product.delete() 
...
class ListingCreateView(APIView):
    ...
    def get(self, request):
        ...
        # Optimization: use select_related and prefetch_related for Cloudinary/Foreign Keys
        listings_queryset = Listing.objects.filter(seller=request.user).select_related(
            'category', 'subcategory', 'country', 'city'
        ).prefetch_related('additional_images').order_by('-created_at')
        ...
        try:
            page_obj = paginator.get_page(page_number)
            #serializer = ListingSerializer(page_obj, many=True)
            # Pass request in context so to_representation can build full Cloudinary URLs
...
class ListingDetailView(APIView):
    ...
    def get_object(self, slug, user):
        return get_object_or_404(
            Listing.objects.prefetch_related('additional_images'), 
            slug=slug, 
            seller=user
        )
    ...
    def delete(self, request, slug):
        ...
        listing.delete() # This triggers the 'post_delete' signal to remove main_image from Cloudinary
        return Response(status=status.HTTP_204_NO_CONTENT)
    ...
    def post(self, request):
        ...
        try:
            ...
            send_mail(
                ...
                from_email=f"DiasporaBlack <{EMAIL_HOST_USER}>",
...
class ResetPasswordConfirmView(APIView):
    ...
    def send_security_alert(self, user):
        ...
        send_mail(
            ...
            from_email=f"DiasporaBlack Security <{EMAIL_HOST_USER}>",
